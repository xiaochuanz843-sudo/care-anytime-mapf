#include <cstdlib>
#include <cstdio>   // ANCHORFIX_PORT: fprintf(stderr,...) diagnostics
#include <string>
static int g_amor_best = 1000000000; // L2_RR_PORT record-to-record best-global tracker
#include "LNS.h"
#include "ECBS.h"
#include <queue>

// ---- L2_SPSA_PORT: file-static learned-acceptance-policy state (one MAPF-LNS2 run per process, like g_amor_best) ----
...
// ---- L2_REPAIR_PORT: file-static repair-order-bandit state (screened-arm-set delayed replan-priority bandit) ----
// eps-greedy-EMA over 5 fixed PP replan-priority rules (L2_REPAIR=20; unset/0 = stock random_shuffle).
// Arms: 0=random(stock) 1=longest-haul 2=shortest 3=most-delayed 4=least-delayed.
// DEFAULT arm set "0,2,3,4" EXCLUDES arm 1 (empirically adverse in the development configuration); override with L2_RB_ARMS.
...
static double g_rb2_eps    = 0.15;          // L2_RB_EPS
static double g_rb2_alpha  = 0.2;           // L2_RB_ALPHA
static bool   g_rb2_allow[5] = {true,false,true,true,true};   // default L2_RB_ARMS="0,2,3,4" (screened arm set)
static bool   g_rb2_cfged  = false;
static void rb2_cfg()
{
    if (g_rb2_cfged) return;
    g_rb2_cfged = true; const char* s;
    if ((s = std::getenv("L2_RB_EPS")))   g_rb2_eps   = atof(s);
    if ((s = std::getenv("L2_RB_ALPHA"))) g_rb2_alpha = atof(s);
    if ((s = std::getenv("L2_RB_ARMS"))) { for (int a = 0; a < 5; a++) g_rb2_allow[a] = false;
...
// ---- CART_PORT: self-tuning budget-decayed record-to-record (L2_ACCEPT=cart) ----
...
// ---- REPV5_PORT: Beta-Thompson-Sampling repair-order bandit (L2_REPAIR=v5) with rawdelta Bernoulli reward ----
...
static double g_ts_scale = 16.0;            // L2_TS_SCALE: raw SoC-delta -> success-prob scale
static int    g_ts_cfged = 0;
static void ts_cfg(){ if (g_ts_cfged) return; g_ts_cfged = 1; const char* s;
    if ((s = std::getenv("L2_TS_SCALE"))) g_ts_scale = atof(s); }
...
// ---- TRAJ_PORT: anytime incumbent-trajectory dump (L2_TRAJ=<path>) ----
...
bool LNS::run()
{
    static const int    _ad_spsa   = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // L2_SPSA_PORT
    static const int    _ad_spsa_W = [](){ const char* s=std::getenv("L2_SPSA_W"); return s?std::max(1,atoi(s)):64; }();
    static const double _ad_spsa_A = [](){ const char* s=std::getenv("L2_SPSA_A"); return s?atof(s):0.05; }();
    static const double _ad_gap_w  = [](){ const char* s=std::getenv("L2_GAP_W");  return s?atof(s):0.0; }();
    { const char* _tp = std::getenv("L2_TRAJ");   // TRAJ_PORT: per-run open, header once
    ...
    static const int _ad_cart = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const bool _amor_on = [](){ const char* a=std::getenv("L2_ACCEPT"); const char* r=std::getenv("L2_REPAIR"); const char* t=std::getenv("L2_TRAJ");
        return (a && a[0]) || (r && r[0]) || (t && t[0]); }();   // ANCHORFIX_PORT: layer switch (any layer env non-empty)
    ...
    { const char* _ea = std::getenv("L2_ACCEPT"); const char* _er = std::getenv("L2_REPAIR"); // ANCHORFIX_PORT fix G: unknown env values must not silently degrade to stock
      if (_ea && _ea[0] && std::string(_ea)!="rr" && std::string(_ea)!="cart" && std::string(_ea)!="spsa" && std::string(_ea)!="sa" && std::string(_ea)!="ta")
          { fprintf(stderr, "[layer] FATAL: unknown L2_ACCEPT=%s (want rr|cart|spsa|sa|ta)\n", _ea); exit(87); }
      if (_er && _er[0] && std::string(_er)!="20" && std::string(_er)!="v5")
          { fprintf(stderr, "[layer] FATAL: unknown L2_REPAIR=%s (want 20|v5)\n", _er); exit(87); } }
    ...
    if (!succ && initial_solution_runtime < time_limit)
    {
        if (use_init_lns)
        {
            ...
            if (succ) // accept new paths
            {
                ...
                g_amor_best = sum_of_costs; // L2_RR_PORT
            }
            initial_solution_runtime = ((fsec)(Time::now() - start_time)).count();
        }
    ...
    // ---- L2_SPSA_PORT: initialize learned-policy state at the initial solution (spsa only; rr/stock untouched) ----
    if (_ad_spsa) {
        ...
        g_sp_th[3] = _ad_gap_w;              // warm-start GAP weight (default 0 => softplus(0)~0.69 near-greedy)
        g_sp_best_paths.clear();
    }
    if (_ad_cart) { g_cart_vol=0.0; g_cart_prev=sum_of_costs; } // CART_PORT init (runs under cart; moved out of the spsa-only block)

    // ---- ANCHORFIX_PORT: anchor accept/best tracking at the initial solution (ALL init paths; idempotent with spsa init above) ----
    if (_amor_on) {
        g_amor_best = sum_of_costs;                    // ANCHORFIX_PORT: fix A - rr/cart/sa/ta first iteration no longer sees ~1e9 anchor
        g_bestret_paths.resize(agents.size());         // ANCHORFIX_PORT: fix B - BESTRET snapshots the initial solution
        for (size_t _bri = 0; _bri < agents.size(); _bri++) g_bestret_paths[_bri] = agents[_bri].path;
        if (g_traj_open) { g_traj_prev = (long)g_amor_best;   // ANCHORFIX_PORT: TRAJ initial row
    ...
    while (runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
        ...
        runtime = ((fsec)(Time::now() - start_time)).count();
        const int _sp_best_before = g_amor_best;                             // L2_SPSA_PORT: best before this update
        sum_of_costs += neighbor.sum_of_costs - neighbor.old_sum_of_costs;
        if (sum_of_costs < g_amor_best) g_amor_best = sum_of_costs; // L2_RR_PORT
        if (g_traj_open && (long)g_amor_best < g_traj_prev) { g_traj_prev = (long)g_amor_best;  // TRAJ_PORT
            g_traj_os << runtime << "," << g_amor_best << "\n"; }
        if (_ad_cart) { // CART_PORT: EMA of |working-cost move| = trajectory volatility
        ...
        if (_amor_on && g_amor_best < _sp_best_before) {                     // BESTRET_PORT: snapshot best incumbent for UNIVERSAL best-return (all arms)   // ANCHORFIX_PORT: fix C - env-gated (stock: zero snapshots)
            g_bestret_paths.resize(agents.size());
            for (size_t _bri = 0; _bri < agents.size(); _bri++) g_bestret_paths[_bri] = agents[_bri].path;
        }
        if (_ad_spsa) {                                                       // L2_SPSA_PORT: online learned-policy update (spsa only)
            ...
            if (g_sp_wc >= _ad_spsa_W) {
                ...
                if (g_sp_phase == 0) { g_sp_Rplus = R; g_sp_phase = 1; }    // measured theta + c*Delta; now measure theta - c*Delta
                    ...
                    for (int i = 0; i < 4; i++) {
                        g_sp_th[i] += _ad_spsa_A * gsign * (double)g_sp_delta[i];
    ...
    // ---- L2_SPSA_PORT: return the BEST incumbent (spsa accepts uphill => last working solution may be worse) ----
    if (_ad_spsa && !g_sp_best_paths.empty()) {
    ...
    if (_amor_on && !g_bestret_paths.empty()) {   // ANCHORFIX_PORT: fix C - env-gated (stock: zero restores)
    ...
    if (_ad_spsa)   // L2_SPSA_PORT: log learned policy for mechanism/liveness analysis
        cout << "SPSA_THETA: " << g_sp_th[0] << " " << g_sp_th[1] << " " << g_sp_th[2] << " " << g_sp_th[3]
             << " (isi=" << g_sp_isi << " best=" << g_amor_best << ")" << endl;
    {   // L2_REPAIR_PORT: log arm pulls + EMA values for mechanism/liveness analysis
        const char* _rbs = std::getenv("L2_REPAIR");
...
bool LNS::runPP()
{
    static const int    _sata_sa   = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="sa")?1:0; }(); // SATA_PORT: simulated annealing
    static const int    _sata_ta   = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="ta")?1:0; }(); // SATA_PORT: threshold accepting
    static const double _sata_t0   = [](){ const char* s=std::getenv("L2_SA_T0");   return s?atof(s):20.0; }();  // SATA_PORT: SA initial temperature
    static const double _sata_tau0 = [](){ const char* s=std::getenv("L2_TA_TAU0"); return s?atof(s):20.0; }();  // SATA_PORT: TA initial threshold
    static const int _ad_rr = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="rr")?1:0; }(); // L2_RR_PORT
    static const int _ad_delta = [](){ const char* s=std::getenv("L2_RR_DELTA"); return s?atoi(s):0; }();
    static const int    _ad_spsa       = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // L2_SPSA_PORT
    static const double _ad_spsa_snorm = [](){ const char* s=std::getenv("L2_SPSA_SNORM"); return s?atof(s):50.0; }();
    static const int _cart = [](){ const char* s=std::getenv("L2_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const double _cart_d0 = [](){ const char* s=std::getenv("L2_CART_D0"); return s?atof(s):10.0; }();
    static const int _cart_vol = [](){ const char* s=std::getenv("L2_CART_MODE"); return (s && std::string(s)=="vol")?1:0; }();
    static const double _cart_k = [](){ const char* s=std::getenv("L2_CART_K"); return s?atof(s):1.0; }();
    static const double _ad_spsa_c     = [](){ const char* s=std::getenv("L2_SPSA_C");     return s?atof(s):0.6; }();
    auto shuffled_agents = neighbor.agents;
    std::random_shuffle(shuffled_agents.begin(), shuffled_agents.end());
    static const int _ad_rb = [](){ const char* s=std::getenv("L2_REPAIR"); return s?atoi(s):0; }(); // L2_REPAIR_PORT
    static const int _ad_rbv5 = [](){ const char* s=std::getenv("L2_REPAIR"); return (s && std::string(s)=="v5")?1:0; }(); // REPV5_PORT
    if ((_ad_rb == 20 || _ad_rbv5) && !iteration_stats.empty() && !neighbor.old_paths.empty()) // REPV5_PORT
    {   // L2_REPAIR_PORT: replan-only arm pull -- pick a priority rule, re-order shuffled_agents accordingly.
        rb2_cfg(); if (_ad_rbv5) ts_cfg();
        int _arm = -1;
        for (int a = 0; a < 5; a++) if (g_rb2_allow[a] && g_rb2_n[a] == 0) { _arm = a; break; }  // warmup: each allowed arm once
        if (_arm < 0 && _ad_rbv5) {   // REPV5_PORT: Thompson sampling over per-arm Beta posteriors (commits ~proportional to certainty)
            double _best = -1.0;
    ...
    while (p != shuffled_agents.end() && ((fsec)(Time::now() - time)).count() < T)
    {
        ...
        if (!_ad_rr && !_ad_spsa && !_cart && !_sata_sa && !_sata_ta && neighbor.sum_of_costs >= neighbor.old_sum_of_costs) // SATA_PORT // L2_RR_PORT/L2_SPSA_PORT: no greedy early-break
            break;
        remaining_agents--;
        path_table.insertPath(agents[id].id, agents[id].path);
        ++p;
    }
    int _rr_allow; // L2_RR_PORT / L2_SPSA_PORT
    ...
    if (_cart) {                                                            // CART_PORT: budget-decayed record-to-record
        double _frac = (time_limit>0.0) ? (1.0 - runtime/time_limit) : 0.0; if (_frac<0.0) _frac=0.0;
        double _dev = _cart_d0 * _frac;
        if (_cart_vol) { double _vn = g_cart_vol / std::max(1.0,(double)(initial_sum_of_costs - sum_of_distances)/std::max(1,(int)agents.size()));
                          _dev = _cart_d0 * _cart_k * _vn * _frac; }
    ...
    } else if (_ad_spsa) {                                                     // learned delta_eff = softplus(theta . [1, s, u, g]) w/ SPSA perturbation
        double s_feat = std::min(1.0, (double)g_sp_isi / _ad_spsa_snorm);
        double u_feat = (time_limit > 0) ? std::min(1.0, runtime / time_limit) : 0.0;
        double _lb = (double)sum_of_distances;                              // SoC lower bound = sum of individual shortest-path distances
        ...
        double t0 = g_sp_th[0] + sign * _ad_spsa_c * g_sp_delta[0];
        double t1 = g_sp_th[1] + sign * _ad_spsa_c * g_sp_delta[1];
        double t2 = g_sp_th[2] + sign * _ad_spsa_c * g_sp_delta[2];
        double t3 = g_sp_th[3] + sign * _ad_spsa_c * g_sp_delta[3];
        ...
        _rr_allow = _ad_rr ? (g_amor_best + _ad_delta - sum_of_costs) : 0;  // L2_RR_PORT (unchanged)
    }
    if (g_rb2_last >= 0)   // L2_REPAIR_PORT: credit the pulled arm on EVERY outcome (accept, reject, failure)
    {
        bool _rb_acc = (remaining_agents == 0 && neighbor.sum_of_costs <= neighbor.old_sum_of_costs + _rr_allow);
        if (_ad_rbv5) {       // REPV5_PORT: magnitude-aware Bernoulli reward + Beta-TS posterior update
            double _p = 0.0;
...
{
    static const bool _mk_on = [](){ const char* s=std::getenv("L2_MKSPAN"); return s && s[0]; }();   // ANCHORFIX_PORT: fix F - L2_MKSPAN gates the makespan column (stock CSV when unset)
    if (init_lns != nullptr)
    {
    ...
    if (!exist)
    {
        ...
        if (_mk_on) addHeads << ",makespan";   // MKSPAN_PORT   // ANCHORFIX_PORT: fix F - gated by L2_MKSPAN
        addHeads << endl;
        addHeads.close();
    }
    ...
    int _mkspan = 0;   // MKSPAN_PORT: makespan = max over agents of (path length - 1)
    if (_mk_on) for (const auto & _ag : agents) { int _pl = (int)_ag.path.size() - 1; if (_pl > _mkspan) _mkspan = _pl; }   // ANCHORFIX_PORT: fix F - gated by L2_MKSPAN
    ...
    if (_mk_on) stats << "," << _mkspan;   // MKSPAN_PORT   // ANCHORFIX_PORT: fix F - gated by L2_MKSPAN
    stats << endl;
    stats.close();
}
