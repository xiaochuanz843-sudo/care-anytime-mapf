...
enum algorithm { TACKLE_SMALL, RANDOM_SEED, ROULETTE_SEED, CANONICAL, GREEDY, EPSILON, EPSILON_DECAY, UCB, TOPK_GREEDY, TOPK_EPSILON, TOP_EPSILON_DECAY, TOPK_UCB, BERNOULIE, NORMAL };
enum b { BCANONICAL, ADD, REPLACE};
enum intent_strategy { CONSTANT_INTENT_BASED, ROULETTE_INTENT_BASED, GREEDY_INTENT_BASED, TABU_INTENT_BASED, RANDOM_INTENT_BASED };
...
private:
    ...
    bool nonStationaryBandits = true;
    ...
    std::vector<int> beta;
    std::vector<std::vector<int>> counterfactual_alpha;
    std::vector<std::vector<int>> counterfactual_beta;
