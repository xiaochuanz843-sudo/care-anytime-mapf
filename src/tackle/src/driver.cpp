...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		("agentNum", po::value<int>()->default_value(0), "number of agents")
        ...
        //custom
        ("algorithm", po::value<string>()->default_value("greedy"), "choose from canonical, greedy, epislon, ucb, decay-epsilon, topk-epsilon, topk-decay-epsilon, topk-ucb1")
        ("epsilon", po::value<double>()->default_value(0), "epsilon value if algorithm = epsilon, decay-epsilon, topk-epsilon, topk-decay-epsilon")
        ("decay", po::value<double>()->default_value(0), "magnitude of decay if used")
        ("k", po::value<int>()->default_value(0), "number of agents to consider")
        ("regions", po::value<int>()->default_value(0), "number of regions")
        ("b", po::value<string>()->default_value("canonical"), "what to include in roulette wheel: canonical (ranodmwalk, random, intersection), add (_,_,_,bernoulie), replace(bernoulie, intersection, random)")
