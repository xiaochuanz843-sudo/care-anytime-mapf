...
#include <string>
static int g_amor_best = 1000000000; // BL_RR_PORT record-to-record best-global tracker
#include "LNS.h"
#include "ECBS.h"
#include <queue>

// ---- BL_SPSA_PORT: file-static learned-acceptance-policy state (one BALANCE run per process, like g_amor_best) ----
...
// ---- BL_REPAIR_PORT: file-static repair-order-bandit state (screened-arm-set delayed replan-priority bandit) ----
// eps-greedy-EMA over 5 fixed PP replan-priority rules (BL_REPAIR=20; unset/0 = stock random_shuffle).
// Arms: 0=random(stock) 1=longest-haul 2=shortest 3=most-delayed 4=least-delayed.
// DEFAULT arm set "0,2,3,4" EXCLUDES arm 1 (empirically adverse in the development configuration); override with BL_RB_ARMS.
...
static double g_rb2_eps    = 0.15;          // BL_RB_EPS
static double g_rb2_alpha  = 0.2;           // BL_RB_ALPHA
static bool   g_rb2_allow[5] = {true,false,true,true,true};   // default BL_RB_ARMS="0,2,3,4" (screened arm set)
static bool   g_rb2_cfged  = false;
static void rb2_cfg()
{
    if (g_rb2_cfged) return;
    g_rb2_cfged = true; const char* s;
    if ((s = std::getenv("BL_RB_EPS")))   g_rb2_eps   = atof(s);
    if ((s = std::getenv("BL_RB_ALPHA"))) g_rb2_alpha = atof(s);
    if ((s = std::getenv("BL_RB_ARMS"))) { for (int a = 0; a < 5; a++) g_rb2_allow[a] = false;
...
// ---- CART_PORT: self-tuning budget-decayed record-to-record (BL_ACCEPT=cart) ----
...
// ---- REPV5_PORT: Beta-Thompson-Sampling repair-order bandit (BL_REPAIR=v5) with rawdelta Bernoulli reward ----
...
static double g_ts_scale = 16.0;            // BL_TS_SCALE: raw SoC-delta -> success-prob scale
static int    g_ts_cfged = 0;
static void ts_cfg(){ if (g_ts_cfged) return; g_ts_cfged = 1; const char* s;
    if ((s = std::getenv("BL_TS_SCALE"))) g_ts_scale = atof(s); }
...
// ---- TRAJ_PORT: anytime incumbent-trajectory dump (BL_TRAJ=<path>) ----
...
bool LNS::run()
{
    static const int    _ad_spsa   = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // BL_SPSA_PORT
    static const int    _ad_spsa_W = [](){ const char* s=std::getenv("BL_SPSA_W"); return s?std::max(1,atoi(s)):64; }();
    static const double _ad_spsa_A = [](){ const char* s=std::getenv("BL_SPSA_A"); return s?atof(s):0.05; }();
    static const double _ad_gap_w  = [](){ const char* s=std::getenv("BL_GAP_W");  return s?atof(s):0.0; }();
    { const char* _tp = std::getenv("BL_TRAJ");   // TRAJ_PORT: per-run open, header once
    ...
    static const int _ad_cart = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const bool _amor_on = [](){ const char* a=std::getenv("BL_ACCEPT"); const char* r=std::getenv("BL_REPAIR"); const char* t=std::getenv("BL_TRAJ");
    ...
    { const char* _ea = std::getenv("BL_ACCEPT"); const char* _er = std::getenv("BL_REPAIR"); // ANCHORFIX_PORT fix G: unknown env values must not silently degrade to stock
      if (_ea && _ea[0] && std::string(_ea)!="rr" && std::string(_ea)!="cart" && std::string(_ea)!="spsa" && std::string(_ea)!="sa" && std::string(_ea)!="ta")
          { fprintf(stderr, "[layer] FATAL: unknown BL_ACCEPT=%s (want rr|cart|spsa|sa|ta)\n", _ea); exit(87); }
      if (_er && _er[0] && std::string(_er)!="20" && std::string(_er)!="v5")
          { fprintf(stderr, "[layer] FATAL: unknown BL_REPAIR=%s (want 20|v5)\n", _er); exit(87); } }
    ...
    if (!succ && initial_solution_runtime < time_limit)
    {
        if (use_init_lns)
        {
            ...
            if (succ) // accept new paths
            {
                ...
                g_amor_best = sum_of_costs; // BL_RR_PORT
            }
            initial_solution_runtime = ((fsec)(Time::now() - start_time)).count();
        }
    ...
    // ---- BL_SPSA_PORT: initialize learned-policy state at the initial solution (spsa only; rr/stock untouched) ----
    if (_ad_spsa) {
    ...
    while (runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
        ...
        runtime = ((fsec)(Time::now() - start_time)).count();
        const int _sp_best_before = g_amor_best;                             // BL_SPSA_PORT: best before this update
        sum_of_costs += neighbor.sum_of_costs - neighbor.old_sum_of_costs;
        if (sum_of_costs < g_amor_best) g_amor_best = sum_of_costs; // BL_RR_PORT
        ...
        if (_ad_spsa) {                                                       // BL_SPSA_PORT: online learned-policy update (spsa only)
    ...
    // ---- BL_SPSA_PORT: return the BEST incumbent (spsa accepts uphill => last working solution may be worse) ----
    ...
    if (_ad_spsa)   // BL_SPSA_PORT: log learned policy for mechanism/liveness analysis
        cout << "SPSA_THETA: " << g_sp_th[0] << " " << g_sp_th[1] << " " << g_sp_th[2] << " " << g_sp_th[3]
             << " (isi=" << g_sp_isi << " best=" << g_amor_best << ")" << endl;
    {   // BL_REPAIR_PORT: log arm pulls + EMA values for mechanism/liveness analysis
        const char* _rbs = std::getenv("BL_REPAIR");
...
bool LNS::runPP()
{
    static const int    _sata_sa   = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="sa")?1:0; }(); // SATA_PORT: simulated annealing
    static const int    _sata_ta   = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="ta")?1:0; }(); // SATA_PORT: threshold accepting
    static const double _sata_t0   = [](){ const char* s=std::getenv("BL_SA_T0");   return s?atof(s):20.0; }();  // SATA_PORT: SA initial temperature
    static const double _sata_tau0 = [](){ const char* s=std::getenv("BL_TA_TAU0"); return s?atof(s):20.0; }();  // SATA_PORT: TA initial threshold
    static const int _ad_rr = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="rr")?1:0; }(); // BL_RR_PORT
    static const int _ad_delta = [](){ const char* s=std::getenv("BL_RR_DELTA"); return s?atoi(s):0; }();
    static const int    _ad_spsa       = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // BL_SPSA_PORT
    static const double _ad_spsa_snorm = [](){ const char* s=std::getenv("BL_SPSA_SNORM"); return s?atof(s):50.0; }();
    static const int _cart = [](){ const char* s=std::getenv("BL_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const double _cart_d0 = [](){ const char* s=std::getenv("BL_CART_D0"); return s?atof(s):10.0; }();
    static const int _cart_vol = [](){ const char* s=std::getenv("BL_CART_MODE"); return (s && std::string(s)=="vol")?1:0; }();
    static const double _cart_k = [](){ const char* s=std::getenv("BL_CART_K"); return s?atof(s):1.0; }();
    static const double _ad_spsa_c     = [](){ const char* s=std::getenv("BL_SPSA_C");     return s?atof(s):0.6; }();
    auto shuffled_agents = neighbor.agents;
    std::random_shuffle(shuffled_agents.begin(), shuffled_agents.end());
    static const int _ad_rb = [](){ const char* s=std::getenv("BL_REPAIR"); return s?atoi(s):0; }(); // BL_REPAIR_PORT
    static const int _ad_rbv5 = [](){ const char* s=std::getenv("BL_REPAIR"); return (s && std::string(s)=="v5")?1:0; }(); // REPV5_PORT
    if ((_ad_rb == 20 || _ad_rbv5) && !iteration_stats.empty() && !neighbor.old_paths.empty()) // REPV5_PORT
    {   // BL_REPAIR_PORT: replan-only arm pull -- pick a priority rule, re-order shuffled_agents accordingly.
    ...
    while (p != shuffled_agents.end() && ((fsec)(Time::now() - time)).count() < T)
    {
        ...
        if (!_ad_rr && !_ad_spsa && !_cart && !_sata_sa && !_sata_ta && neighbor.sum_of_costs >= neighbor.old_sum_of_costs) // SATA_PORT // BL_RR_PORT/BL_SPSA_PORT: no greedy early-break
            break;
        remaining_agents--;
        path_table.insertPath(agents[id].id, agents[id].path);
        ++p;
    }
    int _rr_allow; // BL_RR_PORT / BL_SPSA_PORT
    ...
    if (_cart) {                                                            // CART_PORT: budget-decayed record-to-record
        ...
        _rr_allow = _ad_rr ? (g_amor_best + _ad_delta - sum_of_costs) : 0;  // BL_RR_PORT (unchanged)
    }
    if (g_rb2_last >= 0)   // BL_REPAIR_PORT: credit the pulled arm on EVERY outcome (accept, reject, failure)
    {
