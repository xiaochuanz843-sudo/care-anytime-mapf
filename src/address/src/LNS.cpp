...
#include <string>
static int g_amor_best = 1000000000; // AD_RR_PORT record-to-record best-global tracker
#include "LNS.h"
#include "ECBS.h"
#include <queue>
#include "LNS.h"
...
// ---- AD_SPSA_PORT: file-static learned-acceptance-policy state (one ADDRESS run per process, like g_amor_best) ----
...
static void rb2_cfg()
{
    ...
    if ((s = std::getenv("AD_RB_EPS")))   g_rb2_eps   = atof(s);
    if ((s = std::getenv("AD_RB_ALPHA"))) g_rb2_alpha = atof(s);
    if ((s = std::getenv("AD_RB_ARMS"))) { for (int a = 0; a < 5; a++) g_rb2_allow[a] = false;
...
// ---- CART_PORT: self-tuning budget-decayed record-to-record (AD_ACCEPT=cart) ----
...
// ---- REPV5_PORT: Beta-Thompson-Sampling repair-order bandit (AD_REPAIR=v5) with rawdelta Bernoulli reward ----
...
static double g_ts_scale = 16.0;            // AD_TS_SCALE: raw SoC-delta -> success-prob scale
static int    g_ts_cfged = 0;
static void ts_cfg(){ if (g_ts_cfged) return; g_ts_cfged = 1; const char* s;
    if ((s = std::getenv("AD_TS_SCALE"))) g_ts_scale = atof(s); }
...
// ---- TRAJ_PORT: anytime incumbent-trajectory dump (AD_TRAJ=<path>) ----
...
bool LNS::run()
{
    static const int    _ad_spsa   = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // AD_SPSA_PORT
    static const int    _ad_spsa_W = [](){ const char* s=std::getenv("AD_SPSA_W"); return s?std::max(1,atoi(s)):64; }();
    static const double _ad_spsa_A = [](){ const char* s=std::getenv("AD_SPSA_A"); return s?atof(s):0.05; }();
    static const double _ad_gap_w  = [](){ const char* s=std::getenv("AD_GAP_W");  return s?atof(s):0.0; }();
    { const char* _tp = std::getenv("AD_TRAJ");   // TRAJ_PORT: per-run open, header once
    ...
    static const int _ad_cart = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const bool _amor_on = [](){ const char* a=std::getenv("AD_ACCEPT"); const char* r=std::getenv("AD_REPAIR"); const char* t=std::getenv("AD_TRAJ");
    ...
    { const char* _ea = std::getenv("AD_ACCEPT"); const char* _er = std::getenv("AD_REPAIR"); // ANCHORFIX_PORT fix G: unknown env values must not silently degrade to stock
      if (_ea && _ea[0] && std::string(_ea)!="rr" && std::string(_ea)!="cart" && std::string(_ea)!="spsa" && std::string(_ea)!="sa" && std::string(_ea)!="ta")
          { fprintf(stderr, "[layer] FATAL: unknown AD_ACCEPT=%s (want rr|cart|spsa|sa|ta)\n", _ea); exit(87); }
      if (_er && _er[0] && std::string(_er)!="20" && std::string(_er)!="v5")
          { fprintf(stderr, "[layer] FATAL: unknown AD_REPAIR=%s (want 20|v5)\n", _er); exit(87); } }
    ...
    if (!succ && initial_solution_runtime < time_limit)
    {
        if (use_init_lns)
        {
            ...
            if (succ) // accept new paths
            {
                ...
                g_amor_best = sum_of_costs; // AD_RR_PORT
            }
            initial_solution_runtime = ((fsec)(Time::now() - start_time)).count();
        }
    ...
    // ---- AD_SPSA_PORT: initialize learned-policy state at the initial solution (spsa only; rr/stock untouched) ----
    if (_ad_spsa) {
    ...
    while (runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
        ...
        runtime = ((fsec)(Time::now() - start_time)).count();
        const int _sp_best_before = g_amor_best;                             // AD_SPSA_PORT: best before this update
        sum_of_costs += neighbor.sum_of_costs - neighbor.old_sum_of_costs;
        if (sum_of_costs < g_amor_best) g_amor_best = sum_of_costs; // AD_RR_PORT
        ...
        if (_ad_spsa) {                                                       // AD_SPSA_PORT: online learned-policy update (spsa only)
    ...
    // ---- AD_SPSA_PORT: return the BEST incumbent (spsa accepts uphill => last working solution may be worse) ----
    ...
    if (_ad_spsa)   // AD_SPSA_PORT: log learned policy for mechanism/liveness analysis
        cout << "SPSA_THETA: " << g_sp_th[0] << " " << g_sp_th[1] << " " << g_sp_th[2] << " " << g_sp_th[3]
             << " (isi=" << g_sp_isi << " best=" << g_amor_best << ")" << endl;
    {   // AD_REPAIR_PORT: log arm pulls + EMA values for mechanism/liveness analysis
        const char* _rbs = std::getenv("AD_REPAIR");
...
bool LNS::runPP()
{
    static const int    _sata_sa   = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="sa")?1:0; }(); // SATA_PORT: simulated annealing
    static const int    _sata_ta   = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="ta")?1:0; }(); // SATA_PORT: threshold accepting
    static const double _sata_t0   = [](){ const char* s=std::getenv("AD_SA_T0");   return s?atof(s):20.0; }();  // SATA_PORT: SA initial temperature
    static const double _sata_tau0 = [](){ const char* s=std::getenv("AD_TA_TAU0"); return s?atof(s):20.0; }();  // SATA_PORT: TA initial threshold
    static const int _ad_rr = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="rr")?1:0; }(); // AD_RR_PORT
    static const int _ad_delta = [](){ const char* s=std::getenv("AD_RR_DELTA"); return s?atoi(s):0; }();
    static const int    _ad_spsa       = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="spsa")?1:0; }(); // AD_SPSA_PORT
    static const double _ad_spsa_snorm = [](){ const char* s=std::getenv("AD_SPSA_SNORM"); return s?atof(s):50.0; }();
    static const int _cart = [](){ const char* s=std::getenv("AD_ACCEPT"); return (s && std::string(s)=="cart")?1:0; }(); // CART_PORT
    static const double _cart_d0 = [](){ const char* s=std::getenv("AD_CART_D0"); return s?atof(s):10.0; }();
    static const int _cart_vol = [](){ const char* s=std::getenv("AD_CART_MODE"); return (s && std::string(s)=="vol")?1:0; }();
    static const double _cart_k = [](){ const char* s=std::getenv("AD_CART_K"); return s?atof(s):1.0; }();
    static const double _ad_spsa_c     = [](){ const char* s=std::getenv("AD_SPSA_C");     return s?atof(s):0.6; }();
    auto shuffled_agents = neighbor.agents;
    std::random_shuffle(shuffled_agents.begin(), shuffled_agents.end());
    static const int _ad_rb = [](){ const char* s=std::getenv("AD_REPAIR"); return s?atoi(s):0; }(); // AD_REPAIR_PORT
    static const int _ad_rbv5 = [](){ const char* s=std::getenv("AD_REPAIR"); return (s && std::string(s)=="v5")?1:0; }(); // REPV5_PORT
    if ((_ad_rb == 20 || _ad_rbv5) && !iteration_stats.empty() && !neighbor.old_paths.empty()) // REPV5_PORT
    {   // AD_REPAIR_PORT: replan-only arm pull -- pick a priority rule, re-order shuffled_agents accordingly.
    ...
    while (p != shuffled_agents.end() && ((fsec)(Time::now() - time)).count() < T)
    {
        ...
        if (!_ad_rr && !_ad_spsa && !_cart && !_sata_sa && !_sata_ta && neighbor.sum_of_costs >= neighbor.old_sum_of_costs) // SATA_PORT // AD_RR_PORT/AD_SPSA_PORT: no greedy early-break
            break;
        remaining_agents--;
        path_table.insertPath(agents[id].id, agents[id].path);
        ++p;
    ...
    int _rr_allow; // AD_RR_PORT / AD_SPSA_PORT
    ...
    if (_cart) {                                                            // CART_PORT: budget-decayed record-to-record
        ...
        _rr_allow = _ad_rr ? (g_amor_best + _ad_delta - sum_of_costs) : 0;  // AD_RR_PORT (unchanged)
    }
    if (g_rb2_last >= 0)   // AD_REPAIR_PORT: credit the pulled arm on EVERY outcome (accept, reject, failure)
    {
